...
There are several trails that we enjoy. They are highlighted in the trailmap below. Note that there are three options, each out-and-back trails corresponding to a little, medium-sized, or big adventure.

```{r, create-abrams-trail-map, cache = TRUE}
t <- create_and_save_trailmap(name = "Abrams", 
                         file_name_1 = "abrams-little-bottoms.gpx",
                         auxiliary_trail_files = "GRSM_TRAILS.shp")
```

```{r, fig.cap="Abrams: Big Adventure Trail Map"}
knitr::include_graphics(here("output", "abrams-trailmap.png"))
```
...
```{r, fig.cap="The road to Abrams Creek", out.width="70%", fig.align="center", fig.align="center"}
