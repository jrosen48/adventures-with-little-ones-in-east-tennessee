...
bb <- getbb("")
bb[1, 1] <- min(t1$X) + 1.5*(min(t1$X) - max(t1$X))  # x min - how far west
bb[1, 2] <- max(t1$X) - 1.5*(min(t1$X) - max(t1$X)) # x max - how far east
bb[2, 1] <- min(t1$Y) + .65*(min(t1$Y) - max(t1$Y)) # y min - how far south
bb[2, 2] <- max(t1$Y) - .65*(min(t1$Y) - max(t1$Y))# ymax - how far north
