my_name = "Interpretive Trail (Frozen Head)" 
file_name = "frozen-head-interpretive-trail.gpx"
lon_multiplier = 1
lat_multiplier = 1
zoom = 15
fig_height = 9.47
...
t + geom_label(data = data.frame(x = -83.6903293217033, y = 35.9507100559683,
                                 label = "Bobwhite ADA Trail"),
               mapping = aes(x = x, y = y, label = label),
               inherit.aes = FALSE, family = "special", color = "#595959", alpha = .8, size = 2.25,
               fill = "lightgray") +
    geom_label(data = data.frame(x = -83.7017017717154, y = 35.9500023954001, label = "Island Loop Trail"),
               mapping = aes(x = x, y = y, label = label),
               inherit.aes = FALSE, family = "special", color = "#595959", alpha = .8, size = 2.25,
               fill = "lightgray") +
    geom_label(data = data.frame(x = -83.6885336717014, y = 35.9559015701754, label = "Uplan Trail Inner Loop"),
               mapping = aes(x = x, y = y, label = label),
               inherit.aes = FALSE, family = "special", color = "#595959", alpha = .8, size = 2.25,
               fill = "lightgray") +
    geom_label(data = data.frame(x = -83.6917829431334, y = 35.9432342755102, label = "Seclusion Bend Trail"),
               mapping = aes(x = x, y = y, label = label),
               inherit.aes = FALSE, family = "special", color = "#595959", alpha = .8, size = 2.25,
               fill = "lightgray") +
